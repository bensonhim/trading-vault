---
title: "Start of a Swing"
date: 2026-07-23
tags: [concepts, wiki, auto-compiled]
mention_count: 13326
sources_count: 1455
compilation_count: 1
promote: false
---

# Start of a Swing

*Concepts — compiled from 13326 mentions across 1455 sources.*

## Mentions

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: Books, videos, and AI research summaries are **second‑hand information**. Only by seeing thousands of examples with your own eyes does your brain develop what psychologists call **encapsulation** — the automatic pattern recognition that lets you spot a start of a swing or a reversal bullish without conscious thought.

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: > *"Do it with a blank mind. Don't presume anything... The moment you do it without any indicator, it will show you where the swing started."*

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: This means the *momentum burst* is the core tradable event. Swings that deliver 20% do not dribble out 1% per day; they explode and are largely done within three to five days. That empirical fact answers hold‑time questions automatically: if you are still holding on day six, you are no longer trading the burst — you are hoping.

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: Are you a swing trader (20% moves), a day trader (liquid $10+ range movers), or a home‑run hunter (300%+ annual winners)? Pick **one** track. Do not mix them on the first dive.

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: - **Swing:** run the built‑in *20% study* (stocks up 20%+ in last 5 days).

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: Save the annotated image. Write 1–3 bullet observations per chart. Example: *"Start of swing = 4% breakout. Combo scan would have caught it. Below 50‑day MA. Neglected 3 months."*

> [[../../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]: - 90% of successful swings started with a 4% breakout.

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: - Swing traders need themes, catalysts, and 9M breakouts.

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: | 6 | **Selecting Trading Strategies** | What strategy fits your capital, lifestyle, and drawdown tolerance? | Scalping, day trading, swing trading, position trading, and buy-and-hold each require different stock-selection filters. |

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: These buckets are **mutually exclusive**: Bucket Zero stocks are removed from Bucket One, and both are removed from Bucket Two. The output is a ranked watchlist, not a setup. You still trade your primary setup (SOS, continuation, reversal, etc.) on stocks that appear in these buckets.

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: - **2,500+ funds**: ~166 names. Primary list for large-size swing trades. Clean, lower-volatility swings where 8–10% moves with large capital are achievable.

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: | **PEITS** | Post-earnings intraday/swing surprise | Surprise moves fast but reverts quickly now because everything prices in within hours |

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: If you are a pattern-based trader, decide: *"My primary setup is momentum burst SOS, buy on the first day."* Then run that setup **only** on stocks selected by your chosen filters (e.g., 2,500+ funds for size, or sub-$1B sugar babies for velocity).

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: - **2,500+ funds**: ~166 names; primary large-size swing universe

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: - The earlier you catch the theme via SOS/9M, the higher the edge

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: - [[15. Start of a Swing Guide|Start of a Swing (SOS) Guide — Momentum Burst]]

> [[../../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]: > *"Mean reversion is the number one factor in the short term. That is what is going to screw up your thinking. That is why you have to always buy at the start of a swing and avoid extended stocks."*

> [[../../03-Guides/stockbee/21. Process Guide.md|21. Process Guide]]: - **Quality control checklist**: Is it a cat, dog, or liquid lava? Is it continuation, consolidation, or start-of-a-swing?

> [[../../03-Guides/stockbee/21. Process Guide.md|21. Process Guide]]: | **MXL** | Start-of-a-swing failure | Already up 76% in one day → catalyst used up. Classify as "cat with exhausted catalyst" → pass |

> [[../../03-Guides/stockbee/21. Process Guide.md|21. Process Guide]]: Apply SOS filter (Start-of-a-Swing)

> [[../../03-Guides/stockbee/22. Reversals Guide.md|22. Reversals Guide]]: | **10:30 AM – 3:30 PM** | Free time / low-value for swing entries |

> [[../../03-Guides/stockbee/22. Reversals Guide.md|22. Reversals Guide]]: Reversal bullish is a **low-risk, high-probability mean-reversion swing setup**. It is not about catching falling knives indiscriminately; it is about waiting for a **confirmed exhaustion pattern** (hammer/doji at the end of 3–5 days of selling) on **institutional-quality stocks**, and entering with a **sub-2.5% stop** in the **final minutes of the day**.

> [[../../03-Guides/stockbee/23. Bearish Trading Guide.md|23. Bearish Trading Guide]]: | **CRWV** | Nov 2025 | SOS short, negative catalyst | Stop never entered; went straight down; closed third day |

> [[../../03-Guides/stockbee/23. Bearish Trading Guide.md|23. Bearish Trading Guide]]: - Then wait for a **follow-through SOS (start of a swing)** bar on the next 2–5 days

> [[../../03-Guides/stockbee/23. Bearish Trading Guide.md|23. Bearish Trading Guide]]: | **Entry trigger** | Start-of-a-swing (SOS) after a negative catalyst has occurred; avoid first-leg panic |

> [[../../03-Guides/stockbee/23. Bearish Trading Guide.md|23. Bearish Trading Guide]]: - Guide 09 — Momentum Burst (Bearish): short-side momentum burst uses similar SOS and range-expansion logic

> [[../../03-Guides/stockbee/Chart Fanatics - STEAL This $100 Million Dollar Trading Strategy (Used by Market Wizards)_subs.md|Chart Fanatics - STEAL This $100 Million Dollar Trading Strategy (Used by Market Wizards)_subs]]: **Example — ANF (Abercrombie & Fitch):** New CEO, 244% profit growth turnaround. Stock went from $23 to $200+ (700–800% move over 6–7 months). Example — RAIL: 266% in 2.5 months in a cyclical upswing.

> [[../../03-Guides/stockbee/DEP-Delayed-Reaction-EP.md|DEP-Delayed-Reaction-EP]]: | Qualifier 1 | **SOS** — must be start of swing (first valid buy opportunity) |

> [[../../03-Guides/stockbee/EP-9-Million.md|EP-9-Million]]: - 10% swing with 150% position size = 15% account move

> [[../../03-Guides/stockbee/EP-9-Million.md|EP-9-Million]]: | Qualifier 1 | **SOS** (start of swing) — non-negotiable |

> [[../../03-Guides/stockbee/EP-9-Million.md|EP-9-Million]]: | Entry | OPG, opening range breakout, or first SOS pullback |

> [[../../03-Guides/stockbee/EP-9-Million.md|EP-9-Million]]: 3. **SOS is non-negotiable** — never enter without start of swing

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: # SOS / Dollar Breakout / 4% Breakout

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: SOS (Start of Swing) is the **universal entry qualifier** across all StockBee setups. It's not a standalone strategy — it's the trigger mechanism. Two scan-based variants exist:

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: - **Dollar Breakout (SOS)**: close minus open ≥ $0.90, volume ≥ 100K — finds low-risk intraday entries

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: > "SOS is required for everything in this field. That is a requirement." — 2026-05-29

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: ### SOS — Start of Swing

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: - **First day** of a new directional move — never buy 2nd, 3rd, 4th day of an existing swing

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: - SOS is both a **context** (where are we in the move?) and a **qualifier** (is this a valid entry?)

> [[../../03-Guides/stockbee/SOS-Breakout.md|SOS-Breakout]]: ### Dollar Breakout (SOS-425 variant)

## Related Concepts

*(To be enriched)*

## Contradictions & Gaps

*(To be detected)*

## Sources

- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 1.md|How to Get Started In Trading Guide-Part 1]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 2.md|How to Get Started In Trading Guide-Part 2]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 3.md|How to Get Started In Trading Guide-Part 3]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 4.md|How to Get Started In Trading Guide-Part 4]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 5.md|How to Get Started In Trading Guide-Part 5]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 6.md|How to Get Started In Trading Guide-Part 6]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 7.md|How to Get Started In Trading Guide-Part 7]]
- [[../03-Guides/stockbee/01. How to Get Started/How to Get Started In Trading Guide-Part 8.md|How to Get Started In Trading Guide-Part 8]]
- [[../03-Guides/stockbee/02. How to Trade Breakouts Guide/How to Trade Breakouts Guide-Part 2.md|How to Trade Breakouts Guide-Part 2]]
- [[../03-Guides/stockbee/03. Bullish Momentum Burst Guide/Bullish Momentum Burst Guide-Section 6.md|Bullish Momentum Burst Guide-Section 6]]
- [[../03-Guides/stockbee/04. TI65 Guide/04. TI65 Guide.md|04. TI65 Guide]]
- [[../03-Guides/stockbee/06. DT Guide/06. DT Guide.md|06. DT Guide]]
- [[../03-Guides/stockbee/08. Anticipation (ANTS) Guide/08. Anticipation (ANTS) Guide.md|08. Anticipation (ANTS) Guide]]
- [[../03-Guides/stockbee/13. Episodic Pivots Guide/13. Episodic Pivots Guide.md|13. Episodic Pivots Guide]]
- [[../03-Guides/stockbee/14. Sugar Babies Guide/14. Sugar Babies Guide.md|14. Sugar Babies Guide]]
- [[../03-Guides/stockbee/15. Start of a Swing Guide/15. Start of a Swing Guide.md|15. Start of a Swing Guide]]
- [[../03-Guides/stockbee/16. DEP Guide/16. DEP Guide.md|16. DEP Guide]]
- [[../03-Guides/stockbee/18. Mind Clarity Guide/18. Mind Clarity Guide.md|18. Mind Clarity Guide]]
- [[../03-Guides/stockbee/19. Deep Dive Guide.md|19. Deep Dive Guide]]
- [[../03-Guides/stockbee/20. Stock Selection Guide.md|20. Stock Selection Guide]]