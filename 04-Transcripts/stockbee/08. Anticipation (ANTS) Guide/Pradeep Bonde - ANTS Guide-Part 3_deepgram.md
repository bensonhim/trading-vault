---
title: Pradeep Bonde - ANTS Guide-Part 3
source: https://stockbee-videos.b-cdn.net/antspart3/antspart3.mp4
language: en
backend: deepgram
duration_seconds: 0.0
date: null
tags:
- transcript
- en
- deepgram
---

# Pradeep Bonde - ANTS Guide-Part 3

**Source:** https://stockbee-videos.b-cdn.net/antspart3/antspart3.mp4  
**Language:** en  
**Backend:** deepgram  
**Duration:** 00:00.000

## Summary

Part 3 covers the two entry methods for anticipation setups — manual monitoring and Buy Stop Limit Orders. Key points:

- **Method A (recommended) — Active Morning Monitoring**: Put 3–5 filtered stocks in your brokerage (Interactive Brokers); watch like a hawk in the first 10–15 minutes; enter on even a **1% positive move** (1% is already range expansion)
- **Why early entry matters**: "If you wait for a stock to breakout like this 4% or $1 and then buy, you wasted all your effort because then why did you create this watch list?"
- **Breakouts cluster at open**: Stock breaks out in first 15–20 minutes because many momentum traders watch the same lists
- **Method B — Buy Stop Limit Order (BSLO)**: Set trigger at yesterday's close + 1% (e.g., $26.25 for ~$26 stock), limit cap at $26.50, stop at $25.80; placed before market open
- **BSLO protections**: If stock gaps up above limit ($26.50), order not activated (bad fill avoided); if stock gaps down, order not triggered (natural downside protection)
- **A 1% positive move is your early entry signal**: "As soon as you see a move of even 1%, I get into these stocks"
- **Next video**: Trade management after entry

See curated note: [[08. Anticipation (ANTS) Guide|ANTS Guide]]

## Full Transcript

So once you have a watch list which you have created, you have opportunities in a stocks which has a strong momentum, which is setting up for a possible breakout.

And if the breakout is going to happen, you are definitely going to buy.

Right?

So you have this set of stocks.

Now what do you do with this kind of a set of stocks?

Now if you wait for a stock to breakout like this 4% or $1 and then buy, you wasted all your effort because then why did you create this watch list?

You could have run just your dollar breakout or 4% breakout scan and entered them.

So there is no real benefit to doing this.

The real benefit to doing this is if you can enter this in anticipation of a breakout.

So So before the breakout itself happens, if you can enter this particular stock in hope that it is going to breakout and then you're already in it, you're going to get a closer stock.

Right?

That's the idea.

The second best idea or second best way to do this is once you have these stocks, is you take these stocks and put them in your brokerage platform.

So, if you're using interactive broker, then I put these stocks in interactive broker, Right?

And create orders for them.

Buy orders for them and keep it ready as the market opens.

As soon as you see this breaking out, like this was like yesterday, this was 1.03.

As soon as you see a move of even 1%, I get into these stocks.

Okay?

Now this is again this is for aggressive trader.

If you know what you're doing, this works.

Because if that stock is breaking out and you enter in the first five or ten minutes and if it just takes off, then you get a very aggressive and a close entry.

If you don't do this, this way then you are always going to have a wider stop.

That is the best way to do it, which is like basically take this stock, put them in your IP interface, watch them like a hawk in the morning in the first ten, fifteen minutes because anything which is going to break out of this kind of list is going to break out in the morning in the first fifteen to twenty minutes because a lot of people are watching these stocks.

Every single one of the momentum trader who follows momentum trading is going to have a similar list.

So as soon as they start breaking out, there is a other people get into them.

So earlier you can enter, the better it is.

That's one way of entering.

And I that is my recommended way of doing it.

If you're going to do anticipation, which is to, like, take this list, take it to your brokerage platform, work out where you're going to buy it as soon as it starts breaking out.

Not wait for a complete 4% on a dollar breakout.

If you wait for, say this stock is up or is down 1%, right?

And yesterday.

Even if there's a 1% move, that's a range expansion.

If a 1% move to the positive side, it is going to be a range expansion.

And, that is what you want to get into as soon as possible and then put a stop, right?

So, that is the best way to do this.

There is a second way you can do this.

The second way to do it is what is called as a buy stop limit order.

So, in a buy stop limit order, let's assume that this is the stock, which is a good stock and you are very convinced that this is the stock I want to enter.

And you want to enter as soon as it starts making a move of 1%.

Right?

So if you calculate 1% of this move, it will be around 25¢ or 30¢.

So as soon as it goes up $25.26.25, you want to be entering the stock as soon as it is up, it is at 26.25.

Okay?

So, 26.25 is what is your entry point.

You're just adding some 1% to the stock's price.

Then you create a BSLO order.

BSLO order is a kind of order which you can create in interactive broker.

It is called a buy stop limit order.

So what it will say is that as soon as these stocks goes about 26.25, the stock the order becomes an active order.

And you can specify that because the stock can, like, can I gap up and then your order is going to get filled at a very bad thing?

So you can decide.

This is the trigger point and you can decide an upper limit for a BSL order.

So, you decide that I am willing to buy this stock at 26.25 and the highest price which I am willing to pay is 26.5.

Right?

So, in case this stock goes on Monday, about 26.25, your order becomes active.

And the order will be filled anywhere between twenty six point five zero and twenty six point five zero.

If the stock opens about twenty six point five zero, your order is not going to be activated.

Right?

Or if the stock after making 26.25 goes very rapidly, then you are not going to get fulfilled, you will get filled on a partial.

So, buy stop limit order is one of the ways in which you can enter these stocks.

In order to do that, in addition to this, you can also specify for a buy stop limit order.

What is the stop I'm going to be entering?

So, I'm going to enter this stock with 26.25, 26.5.

And my stop, which I'm going to enter on this, is say, 25.8.

Right?

So, I can put all of these in a buy stop limit order.

And, this buy stop limit order has to be put in before the market opens.

So, you have, you can put a buy stop limit order during the market covers also.

But, if your orders are there on these four or five or three stocks, which are buy stop limit order, then at the opening bell or within five minutes or ten minutes, as soon as the stock breaks out, you're already As soon as it goes to 26.25, you're in the stock.

Your stock will be 25.8.

And at best, your worst case scenario is like you're gonna buy till 26.5.

So if it goes up too much, it gaps up, you're not.

If it gaps down, your order is not gonna be filled because the stock price didn't go beyond 26.25.

So, this way, using BSLO, which is buy stop limit order.

BSLO, which is called buy stop limit order.

You can buy stocks like this with a very close entry point.

At the same time, limiting your risk in terms of either it gaps up or it gaps down.

So, buy stop limit order is second way in which you can enter an anticipation setup using this particular watch list.

And in the next set of video, I'm going to show you, once you enter, how you can manage this particular kind of trade to make sure that you profit from this particular trade and not give up the profit which you can make from this kind of trades.

